Ascension Core version 0.13.2 is now available from:

  <https://download.ascension.org/ascension-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/ascension-project/ascension/issues>
...
ECDSA signatures inside Ascension transactions now use validation using
...
Previous versions of Ascension Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ascension Core was to raise the default minimum
relay fee.

Ascension Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ascension Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ascensiond without having to do any manual configuration.
...
Ascension Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ascension Core will relay transactions with
...
entirely in Ascension Core 0.13, and it is at this time undecided whether the
...
Ascension Core automatically creates a hidden service to listen on, without
manual configuration. Ascension Core will also use Tor automatically to connect
...
Ascensiond can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ascension
Core to use floating fees. By default, Ascension Core will use floating
fees.
...
(default: `0.0002` ASN/kB).

At all times, Ascension Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ASN.
Furthermore, Ascension Core will never create transactions paying less than
...
`ascension.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ascensionrpc>
        ...
        # optional bypass ascensiond rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ascensionrpc:<password>
    </Location>
...
Restarting ascensiond will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ascension.conf
...
ascension-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/ascension-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Ascension Core code base has
...
- `ascension-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ascension-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Ascension scripting system that in combination with
...
Ascension transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Ascension Core.
...
The two phases are distinct in the Ascension-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ascension wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ascension.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ascension block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ascensions they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ascensions being spent is exactly
...
- **Increased security for multisig:** Ascension addresses (both P2PKH addresses
  ...
  believed to be provided by Ascension's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ascension's current ledger (called the UTXO set),
  ...
  note that Ascension Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ascension users to individually opt-in to almost any change in the Ascension
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ascension Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ascension
client to Ascension Core 0.13.2 to signal for segwit support. When a
...
Since Ascension Core 0.10.0, nodes have defaulted to only relaying and
...
- `ascension-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ascension-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
